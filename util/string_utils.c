...
char* nssave(int n, const char* const s1, ...)
{
  va_list ap;
  int i;
  int nb = 0; /* the length of the result, hence the number of bytes to allocate */
  char *tstr; /* to use while concatenating */
...
char* nscat(int n, char* s1, ...)
{
  ...
  int  i;
  int  nb = 0; /* the length of the result, hence the number of bytes to allocate */
  char *as;    /* one of the argument strings */
  char *tstr;  /* to use while concatenating */
  char *nstr;  /* the result */
 ...
 */
char* rindex3(char* str, char ch, char* at)
{
 ...
 */
int eat_space (char* string, int at)
{
  string = &string[at];
