 ...
 *
 * @param instList a Dlist of instructions
 * @param gsymtab a global symbol table
 * @param symtab a symbol table
 * @param varIndex the symbol table index for a variable
 * @return the symbol table index of the result register
 */
int emitComputeVariableAddress(DList instList, SymTable Gsymtab, SymTable symtab, char* varName) {
	...
	if (varIndex == SYM_INVALID_INDEX) { // variable isnt in the local table
		...
		offset = (((int)SymGetFieldByIndex(symtab,varIndex,SYMTAB_OFFSET_FIELD) * -4) - 4);
		...
		//offset *= 4;
		char offsetStr[10];
		snprintf(offsetStr,9,"%d",newOffset);
...
int emitComputeArrayAddress(DList instList, SymTable gsymtab, SymTable symtab, char *varName, int subIndex) {
	...
	if (varIndex == SYM_INVALID_INDEX) {
		...
		if (isArrayType(symtab,varTypeIndex)) {
			offset = (((int)SymGetFieldByIndex(symtab,varIndex,SYMTAB_OFFSET_FIELD) * -4) - 4); // changed to 16 here
			int newOffset = offset + (16 - (offset % 16));
			newOffset *= -1;
			char offsetStr[10];
			snprintf(offsetStr,9,"%d",newOffset);
			...
			dlinkAppend(instList,dlinkNodeAlloc(inst));
			// add the offest to the stack base pointer
			inst = nssave(4,"\t\taddq $", offsetStr, ", ", regName);
			dlinkAppend(instList,dlinkNodeAlloc(inst));
			...
			inst = nssave(2,"\t\timulq $-16, ", subRegName); // changed it to 16 here
			dlinkAppend(instList,dlinkNodeAlloc(inst));
